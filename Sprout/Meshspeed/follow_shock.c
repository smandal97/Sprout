...
void set_W( struct domain * theDomain , int reset ){
   ...
   if( t<t_min*eta_on ){
      ...
      rshock = shock_pos*Ls[fastdim]/(L0s[fastdim]/Cs[fastdim]);
      ...
      for( k=k0 ; k<k1 ; ++k ){
         for( j=j0 ; j<j1 ; ++j ){
            for( i=i0 ; i<i1 ; ++i ){
               ...
               r = fabs(c->xi[fastdim]-Cs[fastdim]);
               if( pv>eps*pv_fast && r>rshock && r>r_fast ){ //
                  r_fast   = r;
      ...
      //if(fabs(t/2e-4-1.)<.2  && theDomain->rank==0) printf("CHECK: t=%.4e, marker=%.4e, r=%.4e, rshock=%.4e\n",t,pv_fast,r_fast,rshock);
      //if(theDomain->rank==0) printf("CHECK: t=%.4e, marker=%.4e, r_fast=%.4e, r_perp=%.4e, rshock=%.4e\n",t,pv_fast,r_fast,r_perp,rshock);
