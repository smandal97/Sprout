...
/* These are useful for situations involving mirror symmetry about a face, e.g., a 1D
   explosion with planar symmetry (but solved in 2/3D). For the direction of motion,
   the left hand boundaries incorporate a Neumann BC with zero derivative with the 
   additional condition that the velocity component of the concerned dimension be
   mirrored in the ghost zones. The right hand side boundaries use a Dirichlet BC.
   For transverse directions, both sides use a Neumann BC with zero derivative. */

void initial( double * , double * , double );
void prim2cons( double * , double * , double * , double );

void set_cells_left( struct cell * cR , struct cell * cS , double dl , int theDIM ){
      ...
      for( k=0 ; k<k_end ; ++k ){
         for( j=0 ; j<j_end ; ++j ){
            for( i=0 ; i<i_end ; ++i ){
               ...
               set_cells_left( crecv , csend , dl , theDIM );
