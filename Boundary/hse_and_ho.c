/*BCs for hydrostatic equilibrium between gravity and presssure gradient coupled with homologous outflow of fluid*/
...
void neumann_setcells( struct cell * cR , struct cell * cS0 , struct cell * cS1 , double dl , double dV , double t , int theDIM ){
   memcpy( cR->xi , cS0->xi , 3*sizeof(double) );
   cR->xi[theDIM] += dl;
   initial( cR->prim , cR->xi , t );
   cR->prim[PPP] = 2.*cS0->prim[PPP] - cS1->prim[PPP];
...
void boundary1D_selective_neumann( struct domain * theDomain , int theDIM ){
      ...
      for( k=k_end ; k>=0 ; --k ){
         for( j=j_end ; j>=0 ; --j ){
            for( i=i_end ; i>=0 ; --i ){
               ...
               neumann_setcells( crecv , csen0 , csen1 , dl , dx*dy*dz , t , theDIM );
            }
         }
      }
   }
...
void boundary( struct domain * theDomain ){
   ...
   if( Nxtot > 1 )boundary1D_selective_neumann( theDomain , 0 );
   if( Nytot > 1 )boundary1D_selective_neumann( theDomain , 1 );
   if( Nztot > 1 )boundary1D_dirichlet( theDomain , 2 );
