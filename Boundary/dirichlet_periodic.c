...
void selective_copy( struct cell * crecv , struct cell * buffer ){
   crecv->prim[UU1] = buffer->prim[UU1];
   crecv->prim[UU3] = buffer->prim[UU3];
}
...
void boundary1D_almost_periodic( struct domain * theDomain , int theDIM ){
...
void boundary( struct domain * theDomain ){
   ...
   if( Nxtot > 1 )boundary1D_almost_periodic( theDomain , 0 );
   if( Nytot > 1 )boundary1D_dirichlet( theDomain , 1 );
   if( Nztot > 1 )boundary1D_almost_periodic( theDomain , 2 );
